...
async function sanitizeRepository(inputRepository) {
  ...
  if (inputRepository.w3cjson) {
    const w3cProperties = w3cJSON(inputRepository.w3cjson);
    if (w3cProperties) {
      // make sure the "exposed" property is set
      if (w3cProperties.exposed === undefined) {
        if (outputRepository.isPrivate === true || outputRepository.isArchived === true) {
          // by default, archived or private repos are not exposed
          w3cProperties.exposed = false;
        } else {
          // public
          w3cProperties.exposed = true;
          
        }
      }
      outputRepository.w3cjson = w3cProperties;
    }
  }
