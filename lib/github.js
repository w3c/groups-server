...
async function sanitizeRepository(inputRepository) {
  ...
  if (inputRepository.w3cjson) {
    const w3cjson = w3cJSON(inputRepository.w3cjson);
    if (w3cjson) {
      if (Object.hasOwn(w3cjson, "exposed") === false) {
        if (outputRepository.isPrivate === true || outputRepository.isArchived === true) {
          // by default, archived or private repos are not exposed
          w3cjson.exposed = false;
        } else {
          // public
          w3cjson.exposed == true;
        }
      }
      outputRepository.w3cjson = w3cjson;
    }
  }
