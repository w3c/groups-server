...
function decorateGroup(group) {
  group["@context"] = "https://w3c.github.io/groups/contexts/group.jsonld";
...
async function* listGroups() {
  for await (const v of iterateHAL("groups", "https://api.w3.org/groups", { embed: true }, decorateGroup))
    yield v;
}

/**
 * Return the list of all W3C groups, including those that are not currently open
 * @returns {Generator<object>} The groups
 */
export
async function* listAllGroups() {
  for await (const v of iterateHAL("groups", "https://api.w3.org/groups", { embed: true, status: "all" }, decorateGroup))
    yield v;
}
...
async function* listGroupServices(identifier) {
  ...
  for await (const v of iterateHAL("services",
      `https://api.w3.org/groups/${safeIdentifier}/services`, { embed: false } ))
    yield v;
}
...
async function* listSpecifications() {
  for await (const v of utils.iterateHAL("specifications", "https://api.w3.org/groups/specifications", { embed: true }))
    yield v;
}
...
async function* listGroupSpecifications(identifier) {
  ...
  for await (const v of iterateHAL("specifications", `https://api.w3.org/groups/${safeIdentifier}/specifications`, { embed: true }))
    yield v;
}
