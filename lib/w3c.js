...
function safeW3CJSON(text) {
    ...
    switch (prop) {
      case "group":
        if (Array.isArray(value)) {
          ret.group = value.map(safeGroupID).filter(n => n !== undefined);
        } else if (value) {
          ret.group = [safeGroupID(value)].filter(n => n !== undefined);
        }
        if (ret.group && !ret.group.length) {
          delete ret.group;
        }
        break;
      case "contacts":
      case "repo-type":
      case "shortname":
          ret[prop] = arrayOfString(value);
        if (ret[prop] && !ret[prop].length) {
          delete ret[prop];
        }
        break;
      case "policy":
        if (value === "restricted") {
          ret.policy = "restricted";
        } else if (value === "open") {
          ret.policy = "open";
        }
        break;
      case "exposed":
        ret.exposed = toBoolean(text.exposed);
        if (ret.exposed === undefined) {
          delete text.exposed;
        }
        break;
      default:
  ...
  let entries = 0;
  for (const props of Object.entries(ret)) {
    entries++;
  }
