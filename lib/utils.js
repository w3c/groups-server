...
*/
export async function* iterateHAL(propertyTarget, initialLink, params, objFct = null) {
  ...
  while (nextLink) {
    ...
    if (params?.embed) apiURL.searchParams.set("embed", "1");
    if (params?.status) apiURL.searchParams.set("status", params.status);

    const data = await json(apiURL);

    const collection = params?.embed
